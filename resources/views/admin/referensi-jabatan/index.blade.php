            ...
            <div class="p-4 border-b">
                <form method="GET" class="flex flex-wrap gap-4">
                    ...
                    @if(request('jenis_jabatan') || request('search'))
                        <a href="{{ route('admin.referensi-jabatan.index') }}" class="px-4 py-2 bg-gray-200 text-gray-700 rounded-md hover:bg-gray-300 text-sm">Reset</a>
                    @endif
                </form>
            </div>
                    ...
                    <tbody class="divide-y divide-gray-200">
                        ...
                        @forelse($masterList as $m)
                            ...
                            <tr class="hover:bg-gray-50 cursor-pointer" @if($hasChildren) x-on:click="open[{{ $m->id }}] = !open[{{ $m->id }}]" @endif>
                                ...
                                <td class="px-6 py-4 text-sm text-center">
                                    <a href="{{ route('admin.referensi-jabatan.edit', $m) }}" class="text-yellow-600 hover:text-yellow-900 mr-2" x-on:click.stop>Edit</a>
                                    <form action="{{ route('admin.referensi-jabatan.destroy', $m) }}" method="POST" class="inline" onsubmit="return confirm('Hapus referensi jabatan ini?')" x-on:click.stop>
                                        @csrf @method('DELETE')
                            ...
                            <tr class="hover:bg-gray-50 bg-gray-50/50" x-show="open[{{ $m->id }}]" x-cloak>
                                ...
                                <td class="px-6 py-3 text-sm text-center">
                                    <a href="{{ route('admin.referensi-jabatan.edit', $child) }}" class="text-yellow-600 hover:text-yellow-900 mr-2">Edit</a>
                                    <form action="{{ route('admin.referensi-jabatan.destroy', $child) }}" method="POST" class="inline" onsubmit="return confirm('Hapus referensi jabatan ini?')">
                                        @csrf @method('DELETE')
                                        <button class="text-red-600 hover:text-red-900">Hapus</button>
                                    </form>
                                </td>
                            ...
                            @endif
                        @empty
                        <tr><td colspan="4" class="px-6 py-10 text-center text-gray-500">Tidak ada data referensi jabatan.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
