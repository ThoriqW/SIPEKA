...
<div class="py-6">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="mb-6">
            <h1 class="text-2xl font-semibold text-gray-900">Edit Referensi Jabatan</h1>
            <p class="text-sm text-gray-500 mt-1"><a href="{{ route('admin.referensi-jabatan.index') }}" class="hover:text-gray-700">Referensi Jabatan</a> / Edit</p>
        </div>
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6"
             x-data="{ jenis: '{{ old('jenis_jabatan', $referensiJabatan->jenis_jabatan) }}' }">
            <form action="{{ route('admin.referensi-jabatan.update', $referensiJabatan) }}" method="POST">
                @csrf @method('PUT')
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Nama Jabatan <span class="text-red-500">*</span></label>
                        <input type="text" name="nama_jabatan" value="{{ old('nama_jabatan', $referensiJabatan->nama_jabatan) }}" required
                    ...
                    <div>
                        ...
                        <select name="jenis_jabatan" x-model="jenis" required
                    ...
                    <div x-show="jenis === 'Fungsional'" x-cloak>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Sub Jabatan</label>
                        <select name="parent_id"
                            ...
                            @foreach($parentList as $id => $nama)
                                <option value="{{ $id }}" {{ old('parent_id', $referensiJabatan->parent_id) == $id ? 'selected' : '' }}>{{ $nama }}</option>
                            @endforeach
                        </select>
                ...
                <div class="flex gap-3 mt-6">
                    <a href="{{ route('admin.referensi-jabatan.index') }}" class="px-4 py-2 bg-gray-200 text-gray-700 rounded-md hover:bg-gray-300 text-sm">Kembali</a>
