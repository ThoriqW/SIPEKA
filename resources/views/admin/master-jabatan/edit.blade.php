...
<div class="py-6">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="mb-6">
            <h1 class="text-2xl font-semibold text-gray-900">Edit Master Jabatan</h1>
            <p class="text-sm text-gray-500 mt-1"><a href="{{ route('admin.master-jabatan.index') }}" class="hover:text-gray-700">Master Jabatan</a> / Edit</p>
        </div>
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <form action="{{ route('admin.master-jabatan.update', $masterJabatan) }}" method="POST">
                @csrf @method('PUT')
                <div class="space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Nama Jabatan</label>
                        <input type="text" name="nama_jabatan" value="{{ old('nama_jabatan', $masterJabatan->nama_jabatan) }}" required
                    ...
                    <div>
                        ...
                        <select name="jenis_jabatan" required onchange="document.getElementById('indukField').style.display = this.value === 'Fungsional' ? '' : 'none'"
                            ...
                            @foreach($jenisJabatanList as $val => $label)
                                <option value="{{ $val }}" {{ old('jenis_jabatan', $masterJabatan->jenis_jabatan) == $val ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                        @error('jenis_jabatan')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                    </div>
                    <div id="indukField" style="display: {{ $masterJabatan->jenis_jabatan === 'Fungsional' ? '' : 'none' }};">
                        ...
                        <select name="parent_id"
                            ...
                            @foreach($parentList as $id => $nama)
                                <option value="{{ $id }}" {{ old('parent_id', $masterJabatan->parent_id) == $id ? 'selected' : '' }}>{{ $nama }}</option>
                            @endforeach
                        </select>
