            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="divide-y divide-gray-200" x-data="treeData()">
                    ...
                    <tr data-id="{{ $row['id'] }}"
                        ...
                        {{-- Kebutuhan --}}
                        <td class="px-3 py-2 text-sm text-center {{ $row['type'] == 'unor' ? 'font-medium text-blue-600' : 'text-gray-900' }}">
                            {{ $row['kebutuhan'] ?? 0 }}
                        </td>

                        {{-- Bezetting --}}
                        <td class="px-3 py-2 text-sm text-center font-medium {{ $row['type'] == 'unor' ? 'text-blue-600' : 'text-gray-900' }}">
                            {{ $row['bezetting'] ?? 0 }}
                        </td>

                        {{-- Selisih --}}
                        <td class="px-3 py-2 text-sm text-center font-medium {{ ($row['selisih'] ?? 0) < 0 ? 'text-red-600' : (($row['selisih'] ?? 0) > 0 ? 'text-green-600' : 'text-gray-500') }}">
                            {{ $row['selisih'] ?? 0 }}
                        </td>
