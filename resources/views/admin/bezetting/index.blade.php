    ...
    <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
        <div class="sm:flex sm:items-center sm:justify-between mb-6">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1">Tabel pohon kebutuhan pegawai — UNOR & Jabatan</p>
            </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="divide-y divide-gray-200" x-data="treeData()">
                    ...
                    <tr data-id="{{ $row['id'] }}"
                        ...
                        {{-- Kebutuhan --}}
                        <td class="px-2 py-2 text-sm text-center {{ $row['type'] == 'unor' ? 'font-medium text-blue-600' : 'text-gray-900' }}">
                            {{ $row['kebutuhan'] ?? 0 }}
                        </td>

                        {{-- Bezetting --}}
                        <td class="px-2 py-2 text-sm text-center font-medium {{ $row['type'] == 'unor' ? 'text-blue-600' : 'text-gray-900' }}">
                            {{ $row['bezetting'] ?? 0 }}
                        </td>

                        {{-- Proyeksi Pensiun Thn 1-5 --}}
                        @for($n = 1; $n <= 5; $n++)
